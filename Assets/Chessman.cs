...
namespace Assets
{
    public abstract class Chessman : MonoBehaviour
    {
        ...
        Vector3[] pointsOfPier;
        int[] triangleElementsOfpier;
        ...
        protected void AddPier(bool coping, float high, float radius)
        {
            ...
            {
                PierPoints[a] = new Vector3((float)(0.75f * radius * Math.Sin(2 * Math.PI * a / 24)), high, (float)(0.75f * radius * Math.Cos(2 * Math.PI * a / 24)));
            }
            ...
            int[] PierTriangleElements = new int[]
            {
            ...
            #region old code
            //// Merge triangle elements below ////
            /*if (this.triangleElements == null)
            {
                this.triangleElements = PierTriangleElements;
            }
            else
            {
                int[] TriangleElementsAll = new int[PierTriangleElements.Length + this.triangleElements.Length]; //create new table of TriangleElement (up of figure + pier)

                for (int TtiElL = 0; TtiElL < this.triangleElements.Length; TtiElL++) //copy triangle of up of figure to the new table
                {
                    TriangleElementsAll[TtiElL] = this.triangleElements[TtiElL];
                }

                for (int PiTriElL = 0; PiTriElL < PierTriangleElements.Length; PiTriElL++) //copy points of pier to the new table
                {
                    TriangleElementsAll[PiTriElL + this.triangleElements.Length] = PierTriangleElements[PiTriElL] + this.triangleElements.Length;
                }

                this.triangleElements = TriangleElementsAll;
            }*/
            ...
            {
                Vector3[] PointsAll = new Vector3[this.points.Length + PierPoints.Length]; //crate new table of points (up of figure + Pier)

                for (int PoiL = 0; PoiL < this.points.Length; PoiL++) //copy points of up of figure to the new table
                {
                    PointsAll[PoiL] = this.points[PoiL];
                }

                for (int PiPoiL = 0; PiPoiL < PierPoints.Length; PiPoiL++) //copy points of pier to the new table
                {
                    PointsAll[PiPoiL + this.points.Length] = PierPoints[PiPoiL];
                }

                this.points = PointsAll; //use new table and forget old one
            }*/
            #endregion
            ...
            /*
            Mesh pierMesh = new Mesh();
            pierMesh.Clear();
            pierMesh.vertices = PierPoints;
            pierMesh.triangles = PierTriangleElements;
            ...
            combine[1].mesh = pierMesh;
            combine[1].transform = transform.localToWorldMatrix;

            var meshToReturn = new Mesh();
            meshToReturn.CombineMeshes(combine);
